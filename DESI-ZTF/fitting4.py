...
def num(mhat,k,mu,alpha,beta,L_star,phi_star):
    L = abs_mag_to_L(mhat-k-mu)/L_star
    numerator = 0.4*phi_star/(L**(-alpha-1)+L**(-beta-1))
    return numerator

def Prob_det_NT(m0,b,k,mu,Lmin,Lmax,alpha,beta,L_star,phi_star,Volume,fraction):
    L1 = abs_mag_to_L(m0-k.mean()-mu.mean())/L_star
...
class ln_posterior(object):
    ...
    def __call__(self,m0,c,mhat,k,mu,fraction):
        b = c/m0
        nsamples = len(mhat)
        ...
        #Lmax = jnp.inf
        Prob_detection_N_Total = Prob_det_NT(m0,b,k,mu,Lmin,Lmax,self.alpha,self.beta,self.L_star,self.phi_star,self.Volume,fraction)
        numerator = num(mhat,k,mu,self.alpha,self.beta,self.L_star,self.phi_star)
...
class ln_posterior1(object):
    ...
    def __call__(self,m0,b,mhat,sigma,k,mu,phi_star,L_star,alpha,beta,fraction):
        #b = c/m0
        #phi_star = d/m0
        nsamples = len(mhat)
        ...
        integrand =  fraction*self.Volume*0.4*phi_star/(Lhat**(-alpha-1)+Lhat**(-beta-1))*efficiency*jnp.exp(-exp_term)
        integral = jnp.trapezoid(integrand,jnp.sort(mhat))
        numerator = num(mhat,k,mu,alpha,beta,L_star,phi_star)
        term1 = -integral
        term2 = jnp.sum(jnp.log(numerator))
        term3 = jnp.sum(jnp.log(efficiency))
        term4 = nsamples*jnp.log(fraction*self.Volume)
        #print(term1)
