...
class ln_posterior1(object):
    ...
    def __call__(self,m0,c,x,mhat,k,mu,phi_star,fraction):
        b = c/m0
        #phi_star = d/m0
        nsamples = len(mhat)
        ...
        integrand =  0.4*phi_star/(Lhat**(-self.alpha-1)+Lhat**(-self.beta-1))*efficiency
        integral = fraction*self.Volume*jnp.trapezoid(integrand,jnp.sort(mhat))
        numerator = num(mhat,x,k,mu,self.alpha,self.beta,self.L_star,phi_star)
        term1 = -integral
        term2 = jnp.sum(jnp.log(numerator))
        term3 = jnp.sum(jnp.log(efficiency))
        term4 = nsamples*jnp.log(0.014*self.Volume)
